...
<div
	class="absolute right-0 bottom-0 left-0 flex {gameState.isLeftHanded
		? 'flex-row-reverse'
		: 'flex-row'} h-48 w-full overflow-hidden border-t-4 border-pink-500/20 bg-gradient-to-br from-[#1a0a0a] via-[#301616] to-[#120505] shadow-[0_-15px_40px_rgba(236,72,153,0.15)] select-none"
>
	<!-- Strumming Patterns Area -->
	<div
		class="relative z-30 flex w-16 flex-[0_0_64px] flex-col items-center justify-center {gameState.isLeftHanded
			? 'border-l-[3px]'
			: 'border-r-[3px]'} border-pink-500/30 bg-[#0a0303] shadow-[5px_0_15px_rgba(0,0,0,0.8)]"
	>
		<div class="flex flex-col gap-3">
			<div
				class="flex h-10 w-10 items-center justify-center rounded-lg border transition-all {activeStrumDir ===
				'up'
					? 'scale-110 border-pink-400 bg-pink-500/40 shadow-[0_0_20px_#ec4899] brightness-125'
					: 'border-pink-500/50 bg-gradient-to-b from-pink-500/20 to-transparent shadow-[0_0_15px_rgba(236,72,153,0.3)]'}"
			>
				<span
					class="text-xl font-bold {activeStrumDir === 'up'
			...
			<div
				class="flex h-10 w-10 items-center justify-center rounded-lg border transition-all {activeStrumDir ===
				'down'
					? 'scale-110 border-cyan-400 bg-cyan-500/40 shadow-[0_0_20px_#00f5ff] brightness-125'
					: 'border-cyan-500/50 bg-gradient-to-b from-cyan-500/20 to-transparent shadow-[0_0_15px_rgba(0,245,255,0.3)]'}"
			>
				<span
					class="text-xl font-bold {activeStrumDir === 'down'
						? 'text-white drop-shadow-[0_0_8px_#fff]'
						: 'text-cyan-400'}">↓</span
				>
			</div>
	...
	<div class="relative flex flex-1 overflow-hidden">
		...
		{#each singleDots as fret}
			{#if fret <= c}
				<div
					class="absolute top-1/2 h-5 w-5 {gameState.isLeftHanded
						? 'translate-x-1/2'
						: '-translate-x-1/2'} -translate-y-1/2 rounded-full border border-white/10 bg-gradient-to-br from-white/80 to-slate-300 shadow-[inset_0_2px_5px_rgba(0,0,0,0.4)]"
		...
		{#each doubleDots as fret}
			{#if fret <= c}
				<div
					class="absolute top-1/3 h-5 w-5 {gameState.isLeftHanded
						? 'translate-x-1/2'
						: '-translate-x-1/2'} -translate-y-1/2 rounded-full border border-white/10 bg-gradient-to-br from-white/80 to-slate-300 shadow-[inset_0_2px_5px_rgba(0,0,0,0.4)]"
					style="{gameState.isLeftHanded ? 'right' : 'left'}: {getMidFretPercentage(fret)}%;"
				></div>
				<div
					class="absolute top-2/3 h-5 w-5 {gameState.isLeftHanded
						? 'translate-x-1/2'
						: '-translate-x-1/2'} -translate-y-1/2 rounded-full border border-white/10 bg-gradient-to-br from-white/80 to-slate-300 shadow-[inset_0_2px_5px_rgba(0,0,0,0.4)]"
					style="{gameState.isLeftHanded ? 'right' : 'left'}: {getMidFretPercentage(fret)}%;"
				></div>
		...
		<div class="absolute inset-0 z-20 flex flex-col">
			{#each Array.from({ length: stringsCount }) as _, s}
				...
				<div class="relative flex flex-1 items-center">
					...
					<div
						class="pointer-events-none absolute right-0 left-0 z-10 w-full bg-gradient-to-b from-slate-200 to-slate-500 shadow-[0_3px_5px_rgba(0,0,0,0.6)]"
						style="height: {1.5 + (stringsCount - s) * 0.4}px;"
					></div>

					<!-- The interactive frets row covering this specific string -->
					<div
						class="relative z-20 flex flex-1 self-stretch {gameState.isLeftHanded
							? 'flex-row-reverse'
							: 'flex-row'}"
					>
