...
>
	...
	<button
		onclick={() => (isLeftHanded = !isLeftHanded)}
		class="flex cursor-pointer items-center gap-2 text-[10px] tracking-widest uppercase transition-colors {isLeftHanded ? 'text-pink-400' : 'text-cyan-400'}"
		title="Toggle left-handed guitar mode"
	>
		<svg
			class="h-4 w-4"
			viewBox="0 0 24 24"
			...
			style={isLeftHanded ? 'transform: scaleX(-1);' : ''}
		>
			<path d="M15 3h4a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2h-4M10 17l5-5-5-5M13.8 12H3" />
		</svg>
		<span>{isLeftHanded ? 'LEFTY' : 'RIGHTY'}</span>
	</button>
	<div class="h-5 w-px bg-cyan-500/10"></div>
